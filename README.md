...
Each row below shows an input/output pair for a concept class. The model sees 2-4 such pairs as a support set and must predict the output for a new input it has never seen.
...
The query sequence attends over all support (input, output) pairs via multi-head cross-attention. The support is also aggregated into a global context vector via mean-pooling, which is added as a residual. Both paths are concatenated and passed through an MLP to produce per-position logits.
...
models/
  sc/                     <- same-concept weights (bilstm, fomaml, poscnp x orig/aug)
